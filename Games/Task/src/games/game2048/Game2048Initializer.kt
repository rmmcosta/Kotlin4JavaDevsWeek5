...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        /*val freeCells: Collection<Cell> = board.filter { it == null }
        if (freeCells.isEmpty()) {
            return null
        }
        val freeCell = freeCells.random()
        return freeCell to generateRandomStartValue()*/
        val freeCell = board.filter { it == null }.randomOrNull()
        return freeCell?.let { it to generateRandomStartValue() }
    }
}